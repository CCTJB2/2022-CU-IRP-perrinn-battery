...
block Ro "Input 1: SoC in fraction, Input 2: Temp in Kelvin"
  ...
  Ro_param.ai a_c(epsilon=epsilon_s_c, R=radius_c)
    annotation (Placement(transformation(extent={{10,10},{20,20}})));
  parameter Modelica.Blocks.Interfaces.RealInput epsilon_s_c
    "active material volume fraction of cathode"
    annotation (Placement(transformation(extent={{-110,-30},{-100,-20}})));
  parameter Modelica.Blocks.Interfaces.RealInput epsilon_s_a
  ...
  Ro_param.ai a_a(epsilon=epsilon_s_a, R=radius_a)
    annotation (Placement(transformation(extent={{10,-52},{20,-42}})));
...
equation
  ...
  connect(a_a.y, multiProduct1.u[1]) annotation (Line(points={{20.5,-47},{25.25,
